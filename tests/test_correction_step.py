...
class TestUpdate(unittest.TestCase):
    def test_compass_meas_zero(self):
        ...
        zt = np.zeros((1,1))
        estimator = EKF()
        ht = ekf.update_compass_measurement_model(estimator.xHat)
    ...
    def test_with_gps_measurements(self):
        ...
        estimator = EKF()
        ht = ekf.update_gps_measurement_model(estimator.xHat)
    ...
    def assert_states_equal(self,xHat,statesExpected):
        decimalPlace = 3
        for i in range(3):
            self.assertAlmostEqual(xHat.p.item(i),statesExpected.p.item(i),decimalPlace)
            self.assertAlmostEqual(xHat.q.item(i),statesExpected.q.item(i),decimalPlace)
            self.assertAlmostEqual(xHat.v.item(i),statesExpected.v.item(i),decimalPlace)
            self.assertAlmostEqual(xHat.ba.item(i),statesExpected.ba.item(i),decimalPlace)
            self.assertAlmostEqual(xHat.bg.item(i),statesExpected.bg.item(i),decimalPlace)

    def assert_covariances_equal(self,xHat,expectedP):
        decimalPlace = 3
        for i in range(15):
            for j in range(15):
                self.assertAlmostEqual(xHat.P[i][j],expectedP[i][j],decimalPlace)
